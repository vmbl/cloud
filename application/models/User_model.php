...
class User_model extends CI_Model
{
    ...
    public function registerLivsol($regData = array()) {
        if(!empty($regData)) {
            if(count($regData['allStates']) > 0) {
                ...
                $regData['states'] = $allstates;
                unset($regData['allStates']);
            }

            if(count($regData['allcities']) > 0) {
                $allcities = implode("|", $regData['allcities']);
                $regData['cities'] = $allcities;
                unset($regData['allcities']);
            }

            $this->db->select("id");
            $this->db->from("livsolreg_data");
            $this->db->where("mobile", $regData['mobile']);   
    ...
    public function trackuploads($dataIn = array(), $path = null) {
        ...
        if(count($dataIn) > 0) {
            if(isset($dataIn['brand'])) {
                $in['brand'] = $dataIn['brand'];
            } 
            if(isset($dataIn['type'])) {
                $in['type'] = $dataIn['type'];
            } 
            if(isset($dataIn['latest'])) {
                if($dataIn['latest'] == "on") {
                    $in['latest'] = 1;
                } else {
                    $in['latest'] = 0;
            ...
            if(isset($dataIn['filename'])) {
                $in['filename'] = $dataIn['filename'];               
            }
